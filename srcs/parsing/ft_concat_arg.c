...
/*   Updated: 2024/10/08 16:22:20 by ahadj-ar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_concatenate(t_parse *parse)
{
	...
	while (tmp)
	{
		...
		if (tmp->arg && tmp->arg[0] != '>' && tmp->arg[0] != '<'
			&& tmp->arg[0] != '|' && tmp->arg[0] != '\'' && tmp->arg[0] != ' '
			&& next_tmp && (next_tmp->arg && (next_tmp->arg[0] != ' '
					&& next_tmp->arg[0] != '|' && next_tmp->arg[0] != '<'
					&& next_tmp->arg[0] != '>')))
		{
			new_arg = ft_concat_args(tmp->arg, next_tmp->arg);
			if (!new_arg)
				return ;
			free(tmp->arg);
			tmp->arg = new_arg;
